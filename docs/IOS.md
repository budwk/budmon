...
1. 在 App Store Connect 的当前 App 下创建**消耗型**内购，产品名为「1个永久监测目标」，产品 ID 必须为 `com.budwk.app.budmon.1`，中国大陆价格设为 **¥1.00**，补全展示名称、说明和审核截图并提交审核。App 内以 `Product.displayPrice` 展示实际商店价格，不伪造价格。
