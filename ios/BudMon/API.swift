...
actor API {
    ...
    private static let origin = "https://budmon.budwk.com"
    private static let base = "https://budmon.budwk.com/api"
    private let session: URLSession
    private var tokens: Tokens?
    private var refreshTask: Task<Tokens, Error>?
    init(session: URLSession = .shared) {
        self.session = session
        // Keep sessions from the official server only; never forward a self-hosted token.
        if let origin = Keychain.read("server"), String(data:origin,encoding:.utf8) == Self.origin,
           let data = Keychain.read("tokens") {
            tokens = try? JSONDecoder().decode(Tokens.self, from: data)
        }
    }
    func hasSession() -> Bool { tokens != nil }
    func save(_ value: Tokens) throws {
        try Keychain.write(Data(Self.origin.utf8), key:"server")
